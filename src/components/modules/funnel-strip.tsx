import type { Funnel, FunnelStage } from "@/lib/data/funnel";
...
export function FunnelStrip({ funnel }: { funnel: Funnel }) {
  ...
  // ONE scale across both paths. Separate scales would draw 18 signals the
  // same width as 34 requisitions and hide the thing that matters most: the
  // warm route is bigger at every stage and it is the one nobody works.
  const max = Math.max(
    ...funnel.stages.map((s) => s.n),
    ...funnel.cold.map((s) => s.n),
    1,
  );
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white px-5 pb-4 pt-4">
      ...
      <Track
        name="Warm — they asked us"
        stages={funnel.stages}
        max={max}
        accent="bg-slate-800"
      />
      {funnel.cold.length > 0 && (
        <Track
          name="Cold — we found them"
          stages={funnel.cold}
          max={max}
          accent="bg-slate-400"
        />
      ...
      {/* One sentence, so nobody has to compare fourteen bars to get the point. */}
      {funnel.worstDrop && (
        <p className="mt-3 border-t border-slate-100 pt-2.5 text-[12.5px] text-slate-600">
          Biggest loss is{" "}
...
function Track({
  ...
  return (
    <div className="mt-3">
      <p className="mb-1.5 font-mono text-[10px] uppercase tracking-[0.12em] text-slate-400">
        {name}
      </p>
      <div className="flex items-end gap-1 overflow-x-auto pb-1">
        {stages.map((s, i) => {
          ...
          const empty = s.n === 0;
          // A stage that loses more than half of the one before it is where
          // the path is actually breaking, so it is the only thing in red.
          const bleeding = i > 0 && !empty && s.n < stages[i - 1].n / 2;
          return (
            <div key={s.key} className="flex min-w-[70px] flex-1 flex-col gap-1.5">
              <div className="flex items-baseline gap-1">
                <span
                  className={`font-mono text-[16px] font-medium tabular-nums leading-none ${
              ...
              <div className="h-1.5 w-full overflow-hidden rounded-full bg-slate-100">
                <div
                  className={`h-full rounded-full ${
                    empty ? "" : bleeding ? "bg-rose-400" : i === 0 ? accent : "bg-slate-500"
                  }`}
