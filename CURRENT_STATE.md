...
| Contextual agent work + mailbox send/reply: `/api/ask` with context creates a missionless assignment; `/now/lead/[id]` shows thread, draft, send, and agent results; SMTP send stores RFC822 id; IMAP matches replies; Today drops Sent / They replied; send is limited to an owned `can_send` mailbox | pending | offline `scripts/check-contextual-work.mjs`; lint, production build, tenant-identity; no signed-in check here | Migrations 049–050 not applied; enable CEO mailbox send with the 16 September data-fix; SMTP unverified live; no Gmail API thread confirmation |
