...
| Job Intake off primary nav: sidebar no longer offers it; `/job-intake` stays as a diagnostics page with a banner; mail ingest/scoring/leads/replies untouched; Bob mail-wake is not this change | this PR | lint, production build, tenant-identity; `/job-intake` still 200 in demo; no signed-in check here | Commercial mail exceptions should surface on Today once Bob wake is proven; Today next-move still deep-links here |
