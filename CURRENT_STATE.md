...
| Today follow-up Ask Bob (Nikola, 18 September): one Ask Bob / Dismiss per person card; after Hand to Bob the same card is With Bob + Open thread, the right-hand drawer opens, toast says the answer returns on this case; extra role ids travel with the assignment | this PR | `check:today-slim`; `check:dev-015`; lint; typecheck | Send from Triangle stays on the hero reply card; agents still cannot send mail; In progress stays quiet for waiting work |
