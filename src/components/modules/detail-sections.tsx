    ...
    <div className="grid gap-4 xl:grid-cols-[1fr_360px]">
      <div className="space-y-4">
        ...
        <Card>
          ...
          <CardContent className="space-y-3 text-sm text-slate-700">
            <p>
              <b>Current projects:</b> {company.currentProjects ?? "Unknown"}
            </p>
            <p>
              <b>Pain points:</b> {company.painPoints ?? "Unknown"}
            </p>
            <p>
              <b>Source:</b> {company.sourceUrl ?? "Manual or unknown"}
            </p>
            <p>
              <b>GDPR notes:</b> B2B legitimate interest; source URL/provenance
              should be preserved.
            </p>
          </CardContent>
        </Card>
      ...
      <div className="space-y-4">
        ...
        <SideList
          title="Contacts"
          items={contacts.map(
            (contact) => `${contact.fullName} · ${contact.jobTitle}`,
          )}
        />
        <SideList
          title="Opportunities"
          items={opportunities.map(
            (opportunity) =>
              `${opportunity.title} · ${formatCurrency(opportunity.estimatedValue)}`,
          )}
        />
        <SideList
          title="Tasks"
        ...
        <SideList
          title="Documents"
          items={documents.map(
            (document) => `${document.title} · ${document.sensitivity}`,
          )}
        />
      </div>
    </div>
    ...
    <div className="grid gap-4 xl:grid-cols-[1fr_360px]">
      <Card>
        <CardHeader
          ...
          description={`${contact.jobTitle} at ${company?.name ?? "Unknown company"}`}
        />
        <CardContent className="grid gap-4 md:grid-cols-2">
      ...
      <div className="space-y-4">
        <Card>
          <CardHeader
            title="Overview"
            description={
              opportunity.scopeSummary ?? "Opportunity scope not yet defined."
            }
          />
          <CardContent className="grid gap-4 md:grid-cols-2">
            ...
            <Info
              label="Crew size"
              value={String(opportunity.estimatedCrewSize ?? "n/a")}
            />
            <Info
              label="Start date"
              value={opportunity.expectedStartDate ?? "n/a"}
            />
            <Info
              label="Value"
        ...
        <Card>
          ...
          <CardContent className="space-y-3 text-sm text-slate-700">
            <p>
              <b>Client need:</b> {opportunity.clientNeed ?? "Unknown"}
            </p>
            <p>
              <b>Next step:</b> {opportunity.nextStep ?? "Set next step"}
            </p>
            <p>
              <b>Risk notes:</b> Add pricing, certification, language and
              mobilisation risks here.
            </p>
          </CardContent>
        </Card>
      </div>
