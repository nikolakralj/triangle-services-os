...
Updated 18 September 2026. This week's development comes first; the 10 September
...
| Last code commit | `e61fdbc` Merge PR #25 DEV-013, 16 September; Production shell smoke 17 September; PR #29 (DEV-019 + DEV-020) still unmerged |
...
| Repository schema | Migrations through `051_mailbox_space.sql`; 044–048 applied 11–15 September; 049 applied 17 September; 050 and 051 applied 18 September (NOTIFY after confirm); first-apply backfill stamped 38/38 existing `job_leads` into the shared space (0 left personal) |
| Production version | `a191b2b`, confirmed through `/api/version` on 17 September and still that on 18 September (PR #29 not promoted) |
...
| Send from Triangle (DEV-013): on the Today reply card a person whose own mailbox has sending on sees Send from Triangle → review (To, From, Subject, text) → Send now; SMTP through their own mailbox; the DEV-001 record is written only after the server accepted (AI draft beside final text, recipient, time, channel, follow-up; `sent_via = triangle`, mailbox, Message-ID); a refusal is a ledger entry and "Not sent", never a Sent record; opt-in switch per mailbox under Settings → Mailboxes, owner only, default off | on `main` | `check:dev-013` 18/18; lint 0; type check 0; tenant-identity 0; production build 0 | Live 17 September: 049 applied and first send recorded. No agent path reaches SMTP; `SENT_MESSAGES_RECORDED` stays false. Only the Today reply card has the button; mission follow-up cards keep Open mail |
| Mailbox-observed sent/replied (DEV-019): after job-intake classify, sync reads INBOX + Sent (envelopes, no LLM, INBOX even with a watch label); a send to someone we are working is recorded (`sent_via = outside`, Message-ID, follow-up from the message date); a reply matches In-Reply-To / References / Gmail thread / subject after a send; a new job from the same recruiter is not a reply; DEV-013 Message-IDs are skipped; Today keeps Open mail · Ask Bob · Dismiss | PR #29 (not on Production) | `check:dev-019` 21/21; lint 0; type check 0; tenant-identity 0; production build 0 | Migration 050 applied 18 September. Live mailbox sync owed after #29 is on Production. Recorded outside Triangle remains. Nothing is sent. `SENT_MESSAGES_RECORDED` stays false. |
| Personal mailbox vs shared space (DEV-020): each person sees mail from their connected inbox; new leads stay personal until Share; existing pipeline backfilled shared on first apply of 051; Today Your mail + Job Intake Mine \| Shared; person Sync now is own mailbox only (cron still all); Bob does not wake on unshared personal ingest; Send from Triangle stays that person's address, optional, off by default | PR #29 (not on Production) | `check:dev-020` 21/21; lint 0; type check 0; tenant-identity 0; production build 0 | Migration 051 applied 18 September; 38/38 existing leads stamped shared (0 left personal). Production still `a191b2b`. Merge #29 and promote, then each person connects their mailbox, Sync now, Share when the other should see it. DEV-014 still later. Provider createDraft later. Nothing is sent. |
