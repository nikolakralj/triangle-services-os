      ...
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1">
          <Link
            ...
            className="line-clamp-2 text-sm font-semibold leading-5 text-slate-950 hover:text-sky-700"
          >
            {opportunity.title}
          </Link>
          ...
          <div className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1.5 text-xs text-slate-600">
            ...
            <span className="inline-flex items-center gap-1 truncate">
              <CircleDollarSign className="h-3.5 w-3.5 shrink-0 text-slate-400" />
...
export function PipelineBoard({
  ...
  const filtered = useMemo(
    () =>
      opportunities.filter(
        (opportunity) =>
          (owner === "all" || opportunity.ownerName === owner) &&
  ...
  function onDragEnd(event: DragEndEvent) {
    ...
    fetch(`/api/opportunities/${opportunityId}/stage`, {
      ...
      // Demo/sample rows may not exist in Supabase yet.
    });
  }
