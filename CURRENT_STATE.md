...
| CASE-004 holding chips open Doors on recruiting missions: `?tab=companies#holding-{id}` switches the surface, opens the row and scrolls it; request chips scroll the worker step; source-mission chips still change page | (this change) | 8/8 offline CASE-004 checks; lint, build, tenant-identity pending in this commit | Does not reopen CASE-004. Manual: Hanna DACH Elektromontage, stay on Overview, click a door chip ? Doors tab, row opens |
