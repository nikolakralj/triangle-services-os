...
// stop being valid. Rendered on the Compliance tab of Talent (DEV-011) and on
// the unlisted /documents page; one component so the two cannot drift.
...
export async function ComplianceOverview({
  ...
  /** Preselects a checklist item in the upload panel (`?checklist=`). */
  initialChecklistItemId?: string;
}) {
  const [docs, checklistItems] = await Promise.all([
    listDocuments(orgId, { role }),
    listDocumentChecklist(orgId),
  ]);
    ...
    <div className="space-y-4">
      {canManage ? (
        <DocumentUploadPanel
          ...
          initialChecklistItemId={initialChecklistItemId}
        />
      ) : null}
